...
def get_all_groups():
    ...
    group_list = [group for group in ResourceGroup.objects.filter(is_deleted=0).exclude(group_name__contains='上线')]
    return group_list
...
def all_instances(user,group_name, type=None, db_type=None, tag_codes=None):
    if user.has_perm('sql.query_all_instances'):
        instances = Instance.objects.all()
    else:
        # 先获取用户关联的资源组
