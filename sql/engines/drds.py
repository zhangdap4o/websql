...
class DRDSEngine(EngineBase):
    ...
    def _query(self, db_name=None, sql='',limit_num=0,close_conn=True):
        ...
        try:
            ...
            logger.warning(f"DRDS命令执行报错，语句：{sql}， 错误信息：{traceback.format_exc()}")
    ...
    def execute_workflow(self, workflow, close_conn=True):
        ...
        try:
            ...
            logger.warning(f"DRDS命令执行报错，语句：{statement or sql}， 错误信息：{traceback.format_exc()}")
