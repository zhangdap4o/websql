...
from .models import Users, Instance, SqlWorkflow, SqlWorkflowContent, QueryLog, DataMaskingColumns, DataMaskingRules, \
    ...
    WorkflowAudit, WorkflowLog, ParamTemplate, ParamHistory, InstanceTag, InstanceTagRelations,skip_columns,necessary_columns
...
@admin.register(skip_columns)
class AliRdsConfigAdmin(admin.ModelAdmin):
    list_display = ('column_id', 'colunm_name', 'isenable', 'sys_time')
    exclude = ('group_parent_id', 'group_sort', 'group_level',)
    search_fields = ['colunm_name']

@admin.register(necessary_columns)
class AliRdsConfigAdmin(admin.ModelAdmin):
    list_display = ('column_id', 'colunm_name', 'isenable', 'sys_time')
    exclude = ('group_parent_id', )
    search_fields = ['colunm_name']
