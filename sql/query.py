...
def query(request):
    ...
    ##if drds then
    if db_type["db_type"] in ["drds"]:
       sql_content=sql_content.replace("explain","explain execute ")
    ...
    try:
        ...
        if db_type["db_type"] in ["drds","mycat"]:
            query_check_info = query_engine.query_check(db_name=db_name,instance_name=instance_name,sql=sql_content)
        else:
