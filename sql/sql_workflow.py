...
def check(request):
    ...
    ## shenjun on 2019-12-05 check workflow
    for sqltype in sqlparse.split(sql_content):
        checksql=sql_check_sql(sqltype)
        sql_type=checksql.check_sql_type()
        if group_name.upper().find("DDL")==-1 and group_name.upper().find("DML")==-1:
            pass
        elif group_name.upper().find("DML")!=-1 and sql_type == "DDL":
            result['status'] = 1
            result['msg'] = '上线工作流选择错误！当前脚本是DML上线'
            return HttpResponse(json.dumps(result), content_type='application/json')

        elif group_name.upper().find("DDL")!=-1 and sql_type == "DML":
            result['status'] = 1
            result['msg'] = '上线工作流选择错误！当前脚本是DDL上线'
            return HttpResponse(json.dumps(result), content_type='application/json')
