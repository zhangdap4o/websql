...
class Instance(models.Model):
    ...
    各个线上实例配置
    """
    instance_name = models.CharField('实例名称', max_length=50, unique=True,help_text='英文或数字，分隔符必须下划线 如 uat_mydb')
    type = models.CharField('实例类型', max_length=6, choices=(('master', '主库'), ('slave', '从库')))
    db_type = models.CharField('数据库类型', max_length=20,default='MySQL', choices=DB_TYPE_CHOICES)
    ##tddlnode = models.CharField('TDDL节点', max_length=500)
    host = models.CharField('实例连接', max_length=500)
...
class skip_columns(models.Model):
    ##定义不检查字段名
    column_id = models.AutoField('ID',primary_key=True)
    colunm_name = models.CharField('字段名', max_length=255,help_text='字母必须大写 如:YMDD_DBA')
    isenable = models.BooleanField('是否启用', default=True)
    sys_time = models.DateTimeField('系统修改时间', auto_now=True)
    ...
    class Meta:
        managed = True
        db_table = 'sql_skip_columns'
        verbose_name = '忽略验证字段'
        verbose_name_plural = '忽略验证字段表'




class necessary_columns(models.Model):
    ##建表必须段名
    column_id = models.AutoField('ID',primary_key=True)
    colunm_name = models.CharField('字段名', max_length=255,help_text='字母必须大写 如:YMDD_DBA')
    isenable = models.BooleanField('是否启用', default=True)
    ...
    class Meta:
        managed = True
        db_table = 'sql_necessary_columns'
        verbose_name = '验证建表必有字段'
        verbose_name_plural = '验证建表必有字段表'
